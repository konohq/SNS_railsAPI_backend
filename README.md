...
| `CLOUDINARY_URL` | Active Storage の本番保存先として使う Cloudinary の接続URL。Cloudinary Dashboard から取得します。 |
...
本番環境のプロフィール画像は Cloudinary に保存します。`CLOUDINARY_URL` は `cloudinary://API_KEY:API_SECRET@CLOUD_NAME` の形式で設定してください。
